...
export function ElectionView() {
  ...
  useEffect(() => {
    client
      .get(new GetRequest().setKey(electionKey))
...
function ManageElectionCard({ election, setElection }) {
  ...
  function CloseSettings() {
    const curClose = election.getClose() && election.getClose().toDate(),
      ...
      isoDate = (d) =>
        d &&
        `${padded(d.getFullYear(), 4)}-${padded(d.getMonth() + 1, 2)}-${padded(
          d.getDate(),
          2
        )}`,
      isoTime = (d) =>
        d && `${padded(d.getHours(), 2)}:${padded(d.getMinutes(), 2)}`,
    ...
    function setDate(date) {
      const op = date
          ? // apparently you can only get this off the window...
            new SetClose().setClose(
              window.proto.google.protobuf.Timestamp.fromDate(date)
            )
          : new SetClose(),
        req = new UpdateRequest()
          .setKey(election.getKey())
          .setOperationsList([new UpdateRequest.Operation().setSetClose(op)]);
    ...
    function setFromState() {
      ...
      const ds = (closeDate + "T" + closeTime)
        .split(/\D+/)
        .map((s) => parseInt(s));
      ds[1] -= 1; // adjust month
      setDate(new Date(...ds));
      ...
      return (
        <div>
          <div className="button-group align-right">
            <button className="submit button" onClick={() => setDate(null)}>
              Reopen
            </button>
          </div>
        </div>
      ...
      return (
        <div>
          ...
          <div className="button-group align-right">
            ...
            <button className="submit button" onClick={setFromState}>
              Reschedule
            </button>
            <button
  ...
  function FlagSettings() {
    ...
    return (
      <fieldset>
        ...
        <FlagCheckbox
          name="Hidden Results"
          flag={Election.Flag.RESULTS_HIDDEN}
        />
      </fieldset>
    );
  }
